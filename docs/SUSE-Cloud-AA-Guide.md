...
* Select the default boot option to _**Install SUSE Cloud Admin**_ (shown, or _**Install SUSE Cloud Admin Embedded**_, as appropriate) ![Image of SC3A-boot](file:///home/bgartner/Pictures/docs/images/SC3A-boot.png)
...
![Image of SC3A-destroy](file:///home/bgartner/Pictures/docs/images/SC3A-destroy.png)

* Then wait as the raw image is
  * loaded to the disk you selected, ![Image of SC3A-loading](file:///home/bgartner/Pictures/docs/images/SC3A-loading.png)
  * verified, ![Image of SC3A-verifying](file:///home/bgartner/Pictures/docs/images/SC3A-verifying.png)
  * and then expanded to fill the disk.

* At this point the system will boot from the installed image.
  * _NOTE: This process may take a few minutes since it executes some first boot steps for the configuration of the SUSE Cloud Administration server._ ![Image of SC3A-prep](file:///home/bgartner/Pictures/docs/images/SC3A-prep.png)
...
* **Language** - Select the desired language to be used during the remainder of the installation and for the installed system, then select _**Next**_.  ![Image of SC3A-language](file:///home/bgartner/Pictures/docs/images/SC3A-language.png)

* **Keyboard Configuration** - Select the appropriate _**Keyboard Layout**_ and then select _**Next**_. ![Image of SC3A-keyboard](file:///home/bgartner/Pictures/docs/images/SC3A-keyboard.png)

* **Hostname and Domain Name** - Enter the desired values for _**Hostname**_ and _**Domain Name**_, followed by _**Next**_.
![Image of SC3A-hostname](file:///home/bgartner/Pictures/docs/images/SC3A-hostname.png)
...
* **Network Configuration II** - To modify the network configuration, select _**Change**_ followed by _**Network Interfaces**_ ![Image of SC3A-network2](file:///home/bgartner/Pictures/docs/images/SC3A-network2.png)
...
![Image of SC3A-network-edit](file:///home/bgartner/Pictures/docs/images/SC3A-network-edit.png)
  ...
  * Select _**Hostname/DNS**_ and fill in the IP Addresses for the ordered list of servers providing that information. ![Image of SC3A-dns](file:///home/bgartner/Pictures/docs/images/SC3A-dns.png)
  ...
  * When using a static IP configuration, select _**Routing**_ and enter the _**Default Gateway**_, then select _**OK**_. Then select _**Next**_ to continue and save the network configuration. ![Image of SC3A-routing](file:///home/bgartner/Pictures/docs/images/SC3A-routing.png)
 * _NOTE : Your value may be different than shown, so please adjust according to your local network configurations settings._

* **Clock and Time Zone** - For these settings, select the proper _**Region**_ and _**Time Zone**_, plus select whether your _**Hardware Clock is set to UTC**_ ![Image of SC3A-clock](file:///home/bgartner/Pictures/docs/images/SC3A-clock.png)
  * Then _**Change**_ the Date and Time and NTP configuration, as necessary, making sure to synchronize with an available NTP Server. Select your NTP Server from the list or configure one manually if you have an internal NTP Server available for use. Select _**Accept**_ when finished and _**Next** to save the time setting changes. ![Image of SC3A-ntp](file:///home/bgartner/Pictures/docs/images/SC3A-ntp.png)

* **Proxy Configuration** - If needed, setup the _**Proxy Configuration**_ by selecting _**Enable Proxy**_ and entering the appropriate settings. No changes are needed if you do not require a proxy to access external network resources. Then select _**Finish**_ to save the proxy settings. ![Image of SC3A-proxy](file:///home/bgartner/Pictures/docs/images/SC3A-proxy.png)
...
* **Novell Customer Center Configuration** - To activate the SUSE Linux Enterprise Server operating system for support and to obtain software updates, there are a few options at this point: ![Image of SC3A-ncc](file:///home/bgartner/Pictures/docs/images/SC3A-ncc.png)
  ...
  * when complete type _**q**_ then _**y**_ to exit this browser and return back to the curses menu where it will start adding the proper remote product and update repositories to your system. ![Image of SC3A-Cloud-ncc-w3m-close](file:///home/bgartner/Pictures/docs/images/SC3A-ncc-w3m-close.png)
  * After the proper remote product and update repositories are added to this host, the available updates will be shown and can be applied. Select _**Accept**_ to apply the available updates. ![Image of SC3A-update](file:///home/bgartner/Pictures/docs/images/SC3A-update.png)
    * _NOTE : The Online Update process will take a few moments to complete and also may require two sessions, the first to update the package manager (shown) and the second to update the remaining packages. Then select **OK** to complete the Novell Customer Center Configuration._ ![Image of SC3A-ncc-success](file:///home/bgartner/Pictures/docs/images/SC3A-ncc-success.png)
...
* **NU Configuration** - Used to enable and configure the SUSE [Subscription Management Tool](https://www.suse.com/documentation/smt11/) to mirror necessary updates, a couple of options exist, depending upon your connectivity to the Internet and whether you wish to configure the SMT service:
  ...
  * or enter the NCC _**User**_ and _**Password**_ fields, then _**Test**_ your configuration. If successful, then select _**OK**_ to continue ![Image of SC3A-smt](file:///home/bgartner/Pictures/docs/images/SC3A-smt.png)
  ...
  * Next, you may be asked for a MySQL Database root password in order to proceed. Enter the desired password in the fields provided and select _**OK**_. ![Image of SC3A-mysql](file:///home/bgartner/Pictures/docs/images/SC3A-mysql.png)
  * As part of a functioning SMT setup, creating a system certificate will be required. Select _**Run CA Management**_ to complete this step or _**Skip**_ if you want to defer setup of the SMT server. ![Image of SC3A-certificate](file:///home/bgartner/Pictures/docs/images/SC3A-certificate.png)
  * A prerequisite step for a system certificate is to set the password for the root user, so enter that in both the _**Password**_ and _**Confirm Password**_ fields, then select _**OK**_ ![Image of SC3A-root-password](file:///home/bgartner/Pictures/docs/images/SC3A-root-password.png)
  ...
  * then select _**Next**_ to create the certificate ![Image of SC3A-certificate-create](file:///home/bgartner/Pictures/docs/images/SC3A-certificate-create.png)
  * and the Any remaining operating system updates will now be applied to the system. ![Image of SC3A-ncc-complete](file:///home/bgartner/Pictures/docs/images/SC3A-ncc-complete.png)
  * At this point, the configured SMT service will finish its setup and configuration ![Image of SC3A-smt-complete](file:///home/bgartner/Pictures/docs/images/SC3A-smt-complete.png)
  * then verify and update all its repositories to the latest content. ![Image of SC3A-smt-mirror](file:///home/bgartner/Pictures/docs/images/SC3A-smt-mirror.png)
...
* **Prepare SUSE Cloud 3 Admin** - Regardless of which flavor is used, the signal for a successful completion will denote **Preparation #2 Complete!** (as shown) ![Image of SC3A-prep2](file:///home/bgartner/Pictures/docs/images/SC3A-prep2.png)


## Ready to Use
At this point, the SUSE Cloud Admin Appliance has been prepared according to the relevant sections of the SUSE Cloud Deployment Guide. Login as the the root user your new SUSE Cloud Administration Server where you should see the following guidance as what to do next. ![Image of SC3A-login](file:///home/bgartner/Pictures/docs/images/SC3A-login.png)
